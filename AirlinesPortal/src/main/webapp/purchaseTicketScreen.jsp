...
<body>
	<%
		...
		if(needWaitingList) {
			...
			ps = con.prepareStatement("insert into ticket values ("+(max+1) + ", 30, "+"'"+formatted+"', '"+temp.getString("arrivalApID")+"', '"+temp.getString("departApID")+"', '"+(((String)session.getAttribute("class")).split(","))[0]+"', "+session.getAttribute("price")+", false, false, 'user')");                                
			ps.executeUpdate();
		}
