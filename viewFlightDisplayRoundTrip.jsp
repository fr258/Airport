...
<body>
		<form method="get" action="viewFlightDisplayRoundTrip.jsp">
			Sort by <select name="sorter" size=1>
				...
				<option value="takeoffasc"> takeoff low to high</option>
				<option value="takeoffdesc"> takeoff high to low</option>
				<option value="descentasc">  descent low to high</option>
				<option value="descentdesc"> descent high to low</option>
		...
		if(request.getParameter("sorter") != null) {
			...
			else if(request.getParameter("sorter").equals("takeoffasc")) {
				ps = con.prepareStatement("select *, departTime diff from finalResults order by diff asc");
			}
			else if(request.getParameter("sorter").equals("takeoffdesc")) {
				ps = con.prepareStatement("select *, departTime diff from finalResults order by diff desc");
				
			}
			else if(request.getParameter("sorter").equals("descentasc")) {
				ps = con.prepareStatement("select *, arrivalTime diff from finalResults order by diff asc");
			}
			else if(request.getParameter("sorter").equals("descentdesc")) {
				ps = con.prepareStatement("select *, arrivalTime diff from finalResults order by diff desc");
			}
		...
		if(tempSet.next()) { %>
			<form method="post" action="ticketRegistrationRoundTrip.jsp">
				<table>
					<tr>
					...
					%>
						<td><input type="radio" name="flight" value=<%= tempSet.getInt("id")+","+tempSet.getInt("idb")%> required/></td>
						<td> <%=tempSet.getString("flightPath") %> </td> 
						<td> <%= departTimeStr %> </td> 
						<td> <%= arrivalTimeStr %> </td> 
						<td> <%=String.format("$%.2f", (Float)tempSet.getFloat("price")+(Float)tempSet.getFloat("priceb")) %>   </td> 
					</tr>
					
					<tr><%
						departTime = LocalDateTime.parse(tempSet.getString("departTimeb").replace(" ", "T"));
						departTimeStr = departTime.format(format);	
						
						
						arrivalTime = LocalDateTime.parse(tempSet.getString("arrivalTimeb").replace(" ", "T"));
						arrivalTimeStr = arrivalTime.format(format);		
					%>
						<td></td>
						<td> <%=tempSet.getString("flightPathb") %> </td> 
						<td> <%= departTimeStr %> </td> 
						<td> <%= arrivalTimeStr %> </td> 
						<td> </td> 
					</tr>
				...
				%> 
					<tr>
						<td><input type="radio" name="flight" value= <%=tempSet.getInt("id")+","+tempSet.getInt("idb")%> /></td>
						<td> <%=tempSet.getString("flightPath") %> </td>
						<td> <%= departTimeStr %> </td> 
						<td> <%= arrivalTimeStr %> </td> 
						<td> <%=String.format("$%.2f", (Float)tempSet.getFloat("price")+(Float)tempSet.getFloat("priceb")) %>   </td> 
